	...
	const data: Timeline[] = [
		{
			...
			skills: ["React", ".NET"],
			class:
				"text-sm font-medium me-2 px-2.5 py-0.5 rounded bg-orange-400 text-orange-100",
		...
		},
		{
			...
			class:
				"text-sm font-medium me-2 px-2.5 py-0.5 rounded bg-blue-900 text-blue-300",
		...
		},
		{
			...
			class:
				"text-sm font-medium me-2 px-2.5 py-0.5 rounded bg-teal-900 text-teal-300",
		...
		},
		{
			...
			class:
				"text-sm font-medium me-2 px-2.5 py-0.5 rounded bg-red-700 text-red-300",
		...
		},
		{
			...
			description:
				"Scolarité de la maternelle à la terminale, baccalauréat général.",
			icon: Book,
			skills: [
			...
			class:
				"text-sm font-medium me-2 px-2.5 py-0.5 rounded bg-green-900 text-green-300",
	...
	return (
		<div id="timeline" className="min-h-screen snap-start relative bg-gray-900">
			...
			<div className="flex items-center justify-center min-h-screen p-4 relative z-10">
				<div className="border border-blue-500/30 bg-linear-to-b from-blue-500/30 to-purple-600/30 rounded-2xl shadow-lg p-8">
					<h2 className="text-3xl font-bold text-white mb-6">Mon Parcours</h2>
					<ol
						ref={timelineRef}
						className="relative border-s-2 border-gray-600 ml-6"
					>
						{data.map((item, index) => (
							<TimelineItem
...
function TimelineItem({
	item,
	index,
}: Readonly<{ item: Timeline; index: number }>) {
	return (
		<li className="mb-8 ml-6 relative timeline-item">
			...
			<div className="ml-6">
				...
				{item.description && (
					<p className="text-base text-white mb-4">{item.description}</p>
				)}
